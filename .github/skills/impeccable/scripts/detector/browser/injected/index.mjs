...
if (IS_BROWSER) {
  ...
  const EXTENSION_MODE =
    (MyScript && MyScript.dataset.impeccableExtension === 'true') ||
  ...
  function collectBrowserFindings() {
    ...
    const Disabled = EXTENSION_MODE
      ? window.__IMPECCABLE_CONFIG__?.disabledRules || []
      : [];
    const RuleOk = (id) => !Disabled.length || !Disabled.includes(id);
    ...
    const typoFindings = checkTypography().filter((f) => RuleOk(f.type));
    ...
    const layoutFindings = checkLayout().filter((f) => RuleOk(f.type));
    ...
    const qualityFindings = checkPageQualityDOM().filter((f) => RuleOk(f.type));
