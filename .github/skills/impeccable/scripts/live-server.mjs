...
function loadBrowserScripts() {
  ...
  const detectPaths = [
    path.join(Dirname, 'detector', 'detect-antipatterns-browser.js'),
    path.join(
      Dirname,
      '..',
      '..',
  ...
  const liveScriptParts = resolveLiveBrowserScriptParts(Dirname);
  try {
...
function createRequestHandler({ detectScript, liveScriptParts }) {
  return (req, res) => {
    ...
    if (p === '/modern-screenshot.js') {
      const vendorPath = path.join(Dirname, 'modern-screenshot.umd.js');
      try {
        res.writeHead(200, {
          'Content-Type': 'application/javascript',
...
if (args.includes('stop')) {
  ...
  if (!keepInject) {
    const injectPath = path.join(Dirname, 'live-inject.mjs');
    try {
