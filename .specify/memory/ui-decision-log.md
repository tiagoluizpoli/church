...
| 2026-07-12 | UI-006 | Tailoring day strip | Replace tall rectangular day cells with compact rounded-square cells; expose a visually separated horizontal scrollbar; make every in-cycle date selectable as a date filter, while retaining event-day markers. | Current strip had `h-14 w-11` cells, hid its scrollbar, and only let event-marked dates select. This conflicted with requested filter affordance and visual treatment. | Medium - changes workspace filter interaction and calendar presentation; no API/data-model impact. | Approved |
| 2026-07-13 | UI-007 | Tailoring day strip follow-up | Replace the raw browser scrollbar with the shared ScrollArea treatment, add vertical breathing room so the selected-day ring is never clipped, and change the strip so only dates with actual slot data are selectable/rendered for filtering. | The current strip still looks visually off, the selected day ring is clipped by its scroll container, and selecting a cycle day with no slots produces an empty list that feels broken rather than informative. | Medium - changes day-strip rendering and filter affordance, but stays within the existing tailoring workspace data and interaction model. | Approved |
