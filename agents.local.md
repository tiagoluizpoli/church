...
- **Phase-by-Phase Implementation Loop (CRITICAL)**:
  ...
  - Within each phase (after coding but before declaring the phase complete):
    ...
    - Run `bun run validate:affected` for changed-file linting, affected-package typechecking, and relevant lower-level tests.
    - When the completed user journey is covered by an E2E spec, run that spec once as its story gate: `bun run test:e2e -- tests/[path].spec.ts`.
    - Run `bun run validate` only for deliberate final handoff or merge validation; it includes every test layer and the full E2E suite.
...
```bash
bunx turbo -F server typecheck --only
bunx turbo -F server test:unit --only -- tests/application/db-event-manager.test.ts
bun run validate:affected -- --dry-run
```
...
- `bun run test` is the deliberate complete suite (~5 min), including E2E.
  During implementation, use `bun run validate:affected` and the declared
  story E2E spec instead.
