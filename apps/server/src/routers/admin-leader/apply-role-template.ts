import { TRPCError } from '@trpc/server';
import { z } from 'zod';
import type { ChurchId } from '../../domain/entities/church';
import type { EventId } from '../../domain/entities/event';
import type { RoleTemplateId } from '../../domain/entities/role-template';
import type { UserId } from '../../domain/entities/volunteer';
import { repositories } from '../../infrastructure/repositories/registry';
import { protectedProcedure } from '../../trpc';
import { authorizeLeaderOrAdmin } from './authorize';

export const applyRoleTemplate = protectedProcedure
  .input(z.object({ eventId: z.string(), templateId: z.string() }))
  .mutation(async ({ input, ctx }) => {
    const callerVol = await repositories.volunteers.findByUserIdGlobally(
      ctx.session.user.id as UserId,
    );
    if (!callerVol) {
      throw new TRPCError({
        code: 'UNAUTHORIZED',
        message: 'Active volunteer profile not found',
      });
    }

    const event = await repositories.events
      .getById(callerVol.churchId, input.eventId as EventId)
      .catch(() => null);
    if (!event) {
      throw new TRPCError({ code: 'NOT_FOUND', message: 'Event not found' });
    }

    const authCtx = await authorizeLeaderOrAdmin(
      ctx.session.user.id,
      event.ministryId,
    );
    const churchId = authCtx.churchId as ChurchId;

    const template = await repositories.roleTemplates
      .getById(churchId, input.templateId as RoleTemplateId)
      .catch(() => null);
    if (!template) {
      throw new TRPCError({ code: 'NOT_FOUND', message: 'Template not found' });
    }

    const slots = await repositories.timeSlots.listByEvent(
      churchId,
      input.eventId as EventId,
    );

    let requirementsCreated = 0;
    for (const slot of slots) {
      for (const item of template.items) {
        await repositories.timeSlots.upsertRequirement(churchId, slot.id, {
          roleId: item.roleId,
          requiredCount: item.requiredCount,
        });
        requirementsCreated++;
      }
    }

    return { updatedSlotCount: slots.length, requirementsCreated };
  });
