...
>
> Parked by [Is the volunteer-facing cluster a live track?](https://github.com/tiagoluizpoli/church/issues/13). Reconciliation shrank it to almost nothing: `dashboard-mappers.ts:30` already derives `timingState: 'in_progress' | 'upcoming'`, and `upcoming-assignments-section.tsx:54` already renders an inline "In progress" badge. **The distinction volunteers need is visible today** — only the section split remains, and this item's own prerequisite 3 calls that split a refinement forcing an information-hierarchy revisit. Small, but it buys clarity over a surface that is already clear enough.
>
> **Promotion trigger** — a volunteer reports confusion about what is happening *now* versus later, **or** the dashboard gains a third competing section, at which point the hierarchy has to be revisited anyway and the split rides along.
