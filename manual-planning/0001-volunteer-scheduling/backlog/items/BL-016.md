...
> ## 📌 Rank 10 · Wave 3 (tier 3) · Issue [#27](https://github.com/tiagoluizpoli/church/issues/27) · **Unblocked**
>
...
>
> **Was blocked by [BL-020](./BL-020.md)** on a real edge: both write the delete/cancel semantics. BL-020 splits delete from cancel on `event.status === 'draft'` and owns the branch **only** — keeping today's cancel behaviour byte-for-byte. This item then layers confirm → leader-acknowledgment → shift-scoped notify onto that cancel branch, and **owns the announcement layer**, including making the scheduled-branch dialog read honestly (*"Cancel this service? Volunteers have already been notified…"*), which BL-020 deliberately left alone. ([Cycle event delete/regenerate semantics](https://github.com/tiagoluizpoli/church/issues/12)) **BL-020 shipped 2026-09-10** (`230e308`, #113) — the cancel branch this item layers onto is now settled and byte-for-byte preserved.
>
...
**Status**: Ranked — wave 3, rank 10. Already grilled, unblocked, ready for `/speckit-specify`.
