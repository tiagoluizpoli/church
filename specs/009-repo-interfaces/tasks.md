...
- [ ] T003 Integrate concrete branded identifier definitions and props in existing entities under `apps/server/src/domain/entities/` (`church.ts`, `ministry.ts`, `volunteer.ts`, `role.ts`, `event.ts`, `time-slot.ts`, `assignment.ts`, `availability.ts`, `assignment-audit.ts`), including non-ID identifiers such as `ChurchSlug` where the contract requires branded domain identifiers
...
- [ ] T006A Define named repository input types (for example `CreateEventInput`, `UpdateAssignmentStatusInput`, `CreateAvailabilityInput`) in `apps/server/src/domain/repositories/` so method signatures never rely on inline object typing or vague `data` parameters
...
- [ ] T007 [P] [US1] Define `ChurchRepository` interface in `apps/server/src/domain/repositories/church.repository.ts` — methods: `getById(id: ChurchId): Promise<Church>`, `getBySlug(slug: ChurchSlug): Promise<Church>`
...
- [ ] T009 [P] [US1] Define `RoleRepository` interface in `apps/server/src/domain/repositories/role.repository.ts` — methods: `getById(churchId, id, tx?): Promise<Role>`, `listByMinistry(churchId, ministryId, tx?): Promise<Role[]>` (default alphabetical ascending by role name)
- [ ] T010 [P] [US1] Define `VolunteerRepository` interface in `apps/server/src/domain/repositories/volunteer.repository.ts` — methods: `getById(churchId, id, tx?): Promise<Volunteer>`, `findByUserId(churchId, userId, tx?): Promise<Volunteer | null>`, `listByMinistry(churchId, ministryId, tx?): Promise<Volunteer[]>`, `hasMembershipInMinistry(churchId, volunteerId, ministryId, tx?): Promise<boolean>`, `hasRoleQualification(churchId, volunteerId, roleId, tx?): Promise<boolean>`, `listQualifiedForRole(churchId, ministryId, roleId, tx?): Promise<Volunteer[]>`, `updateStatus(churchId, volunteerId, status, tx?): Promise<void>`
- [ ] T011 [P] [US1] Define `AvailabilityRepository` interface in `apps/server/src/domain/repositories/availability.repository.ts` — methods: `listByVolunteerInRange(churchId, volunteerId, startTime, endTime, tx?): Promise<Availability[]>`, `create(churchId, input: CreateAvailabilityInput, tx?): Promise<Availability>`, `update(churchId, id, input: UpdateAvailabilityInput, tx?): Promise<void>`, `delete(churchId, id, tx?): Promise<void>`
- [ ] T012 [P] [US1] Define `EventRepository` interface in `apps/server/src/domain/repositories/event.repository.ts` — methods: `getById(churchId, id, tx?): Promise<Event>`, `getWithSlots(churchId, id, tx?): Promise<EventWithSlots>`, `listByMinistry(churchId, ministryId, status?, tx?): Promise<Event[]>` (default chronological ascending by event start time), `create(churchId, input: CreateEventInput, tx?): Promise<Event>`, `updateStatus(churchId, id, input: UpdateEventStatusInput, tx?): Promise<void>`
