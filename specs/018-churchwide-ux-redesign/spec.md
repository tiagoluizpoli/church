...
**Amendment (2026-07-07)**: A second, later grilling session (`.plan/grilling/2026-07-07-planning-cycle-lock-unlock-and-nav.md`, handoff at `.plan/handoffs/grill-to-prd-planning-cycle-lock-unlock-and-nav.md`) surfaced two further planning-cycle-screen fixes from live testing that fit this spec's existing "frontend-only, no new domain entities" boundary and are folded in below: (1) restructuring `/scheduling` into a real parent route with role-gated nested children instead of same-route client-side tabs; (2) deduplicating the "locked" status indicator to one canonical location and splitting the combined name+status+period header chip. That same session also grilled a third, related fix — day/event-level forced-override editing of a locked cycle's calendar days, with a leader-acknowledgment gate and scoped volunteer notification (replacing an earlier "unlock the whole cycle" idea) — which is **explicitly excluded from this spec**: it requires a new persisted acknowledgment entity and new backend override paths, which would break this spec's own Constitution gate (no new domain entities, no schema changes). That work is tracked as its own future spec-kit spec and its own backlog entry (BL-016); see the handoff's "Next Step" section.
...
5. **Given** a ChurchAdmin selects a cycle and opens the template library from the planning-cycles screen, **When** they do so, **Then** the URL changes to reflect the selected cycle and the template-library sub-view (e.g. `/scheduling/planning-cycles/:cycleId`), and browser back/forward navigate correctly between them — unlike today, where selecting a cycle and opening the template library both stay under `/scheduling/planning`'s unchanging URL as internal component state.
6. **Given** a Leader or Sub-leader, **When** they navigate to `/scheduling/tailoring` or `/scheduling/builder-events`, **Then** they can reach these directly (e.g. via a bookmarked or shared URL) — both already resolve today, but `/scheduling/builder-events` is a new path replacing the misleadingly-named bare `/scheduling` index.
7. **Given** a user without the ChurchAdmin role attempts to visit `/scheduling/planning-cycles` directly by URL, **When** the route loads, **Then** they are redirected/denied at the route level before the page renders — not only after an in-page query round-trips and fails, as happens today.
8. **Given** the planning-cycle header today shows one combined `<name> [locked] <start> → <end>` chip, and the same "locked" status repeated on `Selected cycle review` and `Calendar review`, **When** the redesigned screen renders, **Then** the header shows the cycle name with its status badge as one chip and the date range as a separate chip, and no other section on the screen repeats the status badge.
...
- Does the nested-route restructure (FR-015-FR-018) change the "locked cycle → read-only step" edge case above? No — that invariant is untouched; `PlanningStep`'s `locked-review` state stays exactly as shipped. Any future ability to edit an individual locked day/event is out of scope for this spec (see the 2026-07-07 Amendment note above); it belongs to its own future spec and does not relax this edge case.
...
- **FR-015** *(2026-07-07 amendment, corrected during `/speckit-plan` — see `research.md` R6)*: `/scheduling/planning` (ChurchAdmin only), `/scheduling/tailoring`, and `/scheduling/builder-events` (Leader/Sub-leader) MUST each enforce their own role guard at the route level (not just via reactive query-level 403s as today). `/scheduling/planning` MUST be renamed `/scheduling/planning-cycles`, and the "Builder events" tab (currently the bare `/scheduling` index) MUST get its own `/scheduling/builder-events` path. These 3 areas already exist as distinct, URL-addressable routes today (`research.md` R6) — this FR closes the role-guard and naming gaps, it does not introduce nested routing from scratch.
- **FR-016** *(2026-07-07 amendment)*: Within `/scheduling/planning-cycles`, the create step and a selected cycle's template/review steps MUST be URL-addressable (e.g. `/scheduling/planning-cycles/new`, `/scheduling/planning-cycles/:cycleId`), so that browser back/forward and deep-linking to a specific cycle work correctly.
- **FR-017** *(2026-07-07 amendment)*: The ambiguous same-route "Back to cycles / Template library / Create cycle" button row MUST be replaced by breadcrumb-style navigation that is visually distinct from in-page action buttons (e.g. "Create cycle" remains a primary action button; moving between the cycle list, a cycle's review, and the template library reads as navigation, not an action).
- **FR-018** *(2026-07-07 amendment)*: The planning-cycle header MUST present the cycle name with its status badge as one chip, and the cycle's date range as a separate chip — replacing today's single combined `<name> [status] <start> → <end>` chip.
- **FR-019** *(2026-07-07 amendment)*: The "locked" status indicator MUST appear in exactly one canonical location (the header chip from FR-018); the redundant status badges today shown on `Selected cycle review` and `Calendar review` MUST be removed.

*No [NEEDS CLARIFICATION] markers: this feature was fully resolved through a dedicated grilling session (`.plan/grilling/2026-07-06-bl014-churchwide-ux-redesign.md`) before this spec was written; open questions were resolved there rather than deferred here. FR-015-FR-019 were resolved through a second grilling session (`.plan/grilling/2026-07-07-planning-cycle-lock-unlock-and-nav.md`) on 2026-07-07; see that session's Answered Questions for the full reasoning trail.*
...
- **SC-007** *(2026-07-07 amendment)*: Every navigation between the planning-cycle screen, the template library, and back changes the URL, and browser back/forward correctly retrace those steps — verified by inspecting the address bar at each step, not just the rendered UI.
- **SC-008** *(2026-07-07 amendment)*: The "locked" status is visible in exactly one place on the planning-cycle screen at any cycle state, confirmed by inspecting the full rendered screen for a locked cycle.
...
- *(2026-07-07 amendment)* Whether `ChurchAdmin` should also be able to view `/scheduling/tailoring` and `/scheduling/builder-events` (e.g. for oversight), in addition to Leader/Sub-leader, was not resolved by the 2026-07-07 grilling session — the user's framing described each screen as belonging to its own audience (Planning-cycles = admin, Tailoring/Builder-events = leader/sub-leader) without stating whether admin access to the latter two is also wanted. FR-015 is written to require only Leader/Sub-leader on those two routes; broadening them to also allow ChurchAdmin is a small, low-risk follow-up if desired, and should be confirmed before or during implementation rather than assumed either way.
- *(2026-07-07 amendment)* The day/event-level forced-override editing feature (locked-day time edits, leader-acknowledgment gate, scoped volunteer notification) discussed in the same 2026-07-07 grilling session is deliberately **not** part of this spec — see the Amendment note at the top of this document and the handoff's "Next Step" section for where that work is tracked.
